...
export default function ManageEmp() {
  ...
  const generatePDF = () => {
    const doc = new jsPDF();
    const tableColumn = ["Name", "Week No", "Weight (kg)", "Height (cm)", "BMI", "Exercise Time (min)", "Meals", "Water Intake (L)", "Sleep Time (h)", "Rest Day"];

    const tableRows = filter.map(item => {
        const heightInMeters = item.height / 100;
        const bmi = (item.weight / (heightInMeters * heightInMeters)).toFixed(2); // BMI calculation
